...
rwlist		: rwlist ',' data16
{
	if( pass2 ) genrword($3.value);
	$$.value = $1.value + 2;
}
		| data16
{
	if( pass2 ) genrword($1.value);
	$$.value = 2;
}
		;
...
instr		: NOP
		...
		| LJMP addr11_16
                                {
                                  $$.value = makeop(lookop(($2.mode.size == 1) ? "ajmp" : "ljmp"),&$2.mode,0);
                                }
                | LCALL addr11_16
                                {
                                  $$.value = makeop(lookop(($2.mode.size == 1) ? "acall" : "lcall"),&$2.mode,0);
                                }
		| JC relative
				{ $$.value = makeop($1.op,&$2.mode,0); }
		| JNC relative
